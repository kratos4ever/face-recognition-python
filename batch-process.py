...
def loadTrainingImages():
	...
	sql = " select a.lan_id,a.id,a.capture_time,a.image,a.processed_time,a.status,a.result from train_img a, " + 
	"(select lan_id, max(id) as id from train_img where lan_id in ("++") group by lan_id) b where a.id = b.id "
